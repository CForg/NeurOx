/* This replicates the code on Page 58 of NNFS usng NeurOx */
...
main() {
	...
	net.AddLayers(new Dense(<4,3>,LinAct));     //add the LinAct layer

	net.SetBatchAndTarget(NoLoss,batch);        // set Loss as "NoLoss" so no target required, feed in batch

    net.SetParameters( vecr(biases|weights) );  // Populate weights and biases (always vectorized then reshaped internally)
